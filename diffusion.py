...
class Diffusion(L.LightningModule):
    def __init__(self, config, tokenizer: transformers.PreTrainedTokenizer):
        ...
        if self.config.algo.backbone == "dit":
            ...
            if (
                getattr(self.backbone.config, "attn_backend", None) == "flex"
                and self.config.model.attn_backend == "sdpa"
            ):
                self.backbone.config.attn_backend = "sdpa"
                for i in self.backbone.backbone.blocks:
                    i.attn_backend = "sdpa"
                self.backbone.backbone.gen_mask(
                    self.config.model.length, self.block_size, attn_backend="sdpa"
                )
        else:
        ...
        if self.var_min:
            self.register_buffer(
                "sampling_eps_min", torch.tensor(self.config.training.sampling_eps_min)
            )
            self.register_buffer(
                "sampling_eps_max", torch.tensor(self.config.training.sampling_eps_max)
            )
    ...
    def _get_parameters(self):
        parameters = [self.backbone.parameters(), self.noise.parameters()]
    ...
    def _validate_configuration(self):
        ...
        if self.config.mode == "sample_eval":
            assert (
                self.config.model.attn_backend != "flex"
            ), "FlexAttention mask not supported at inference."
        if self.config.model.attn_backend == "flex":
            assert (
                self.config.algo.name == "bd3lm"
            ), "Custom FlexAttention mask only supported for BD3LM."

    def to(self, *args, **kwargs):
        self = super().to(*args, **kwargs)
        self.metrics.to(*args, **kwargs)
        if (
            hasattr(self.backbone, "block_diff_mask")
            and self.config.model.attn_backend == "sdpa"
        ):
            self.backbone.block_diff_mask = self.backbone.block_diff_mask.to(
                *args, **kwargs
            )
        elif (
            hasattr(self.backbone, "block_diff_mask")
            and self.config.model.attn_backend == "flex"
        ):
            self.backbone.block_diff_mask = self.backbone.block_diff_mask.to(
                self.device
            )
    ...
    def validation_step(self, batch, batch_idx):
        if self.var_min:
            for noise_clip_start in self.metrics.valid_vars.keys():
                ...
                if (
                    self._check_val_sampling_intvl(sampling_eps_min, sampling_eps_max)
                    == True
                ):
    ...
    def _resample_q_xt(
        ...
        while (perc_masked < sampling_eps_min).any() or (
            ...
            move_indices[regen_idx] = (torch.rand(*x.shape, device=x.device) < p)[
                regen_idx
            ]
    ...
    def q_xt(self, x, p, block_size=None, sampling_eps_min=None, sampling_eps_max=None):
        ...
        if self.config.training.resample and not (
            ...
            xt = xt.reshape(xt.shape[0], -1, block_size)
            xt = self._resample_q_xt(
                x, xt, move_indices, p, block_size, sampling_eps_min, sampling_eps_max
            )
    ...
    def _ddpm_caching_update(self, x, t, dt, p_x0=None):
        ...
        if self.config.sampling.first_hitting:
            ...
            mask = (torch.arange(self.block_size, device=x.device) == ind[:, None]).to(
                x_block.dtype
            )
    ...
    @torch.no_grad()
    def _sample(self, seqlen=None, num_steps=None, eps=1e-5, batch_size_per_gpu=None):
    ...
    def _sample_t(
        ...
        if self.antithetic_sampling:
            offset_b = torch.arange(batch_dims[0] * num_blocks, device=device) / (
                batch_dims[0] * num_blocks
            )
    ...
    def _forward_pass_diffusion(
        self, x0, t=None, sampling_eps_min=None, sampling_eps_max=None
    ):
        if t is None:
        ...
        xt = self.q_xt(
            x0, p, sampling_eps_min=sampling_eps_min, sampling_eps_max=sampling_eps_max
        )
    ...
    def _loss(
        self, x0, attention_mask, t=None, sampling_eps_min=None, sampling_eps_max=None
    ):
    ...
    def _clipped_schedule_search(self):
        ...
        for (eps_min, eps_max), var in self.metrics.valid_vars.items():
            all_vars = torch.tensor(0.0, device=self.device)
            ...
            self.log(
                f"valid_var_{round(eps_min, 2)} - {round(eps_max, 2)}",
                all_vars / len(var),
                on_epoch=True,
                on_step=False,
                sync_dist=True,
        ...
        for stride_num in tqdm(range(num_strides)):
            ...
            if mdlm_semi_ar and stride_num > 0:  # MDLM
                fwd_idx = torch.arange(
                    512 * (stride_num), (512 * (stride_num)) + self.block_size
                )
