...
class DDiTBlock(nn.Module):
    ...
    def get_qkv(self, x, rotary_cos_sin, store_kv=False):
        ...
        if self.kv_cache is not None:
            ...
            self.kv_cache[
                :, self.cache_idx : self.cache_idx + self.block_size
            ] = new_qkv
        ...
        if store_kv:
            ...
            if self.cache_idx >= self.n:
                ...
                self.kv_cache[:, : -self.block_size] = self.kv_cache[
                    :, self.block_size :
                ].clone()
    ...
    def forward(
        self, x, rotary_cos_sin, c, mask=None, sample_mode=False, store_kv=False
    ):
        ...
        if self.adaln:
            ...
            x = bias_dropout_scale_fn(
                self.attn_out(x), None, torch.ones_like(x), x_skip, self.dropout
            )
            x = bias_dropout_scale_fn(
                self.mlp(self.norm2(x)), None, torch.ones_like(x), x, self.dropout
            )
...
class DITBackbone(nn.Module):
    def __init__(self, config: BD3LMConfig):
        ...
        if self.cross_attn:
            self.gen_mask(
                config.model_length, self.block_size, attn_backend=config.attn_backend
            )
    ...
    def forward(
        ...
        if self.cross_attn and not sample_mode:
            logits = logits[:, :n]
            all_hidden_states = [
                hidden_states[:, :n] for hidden_states in all_hidden_states
            ]
...
class BD3LM(transformers.PreTrainedModel):
    ...
    def __init__(self, config: BD3LMConfig):
        ...
        if config.var_min:
            self.register_buffer(
                "sampling_eps_min", torch.tensor(config.sampling_eps_min)
            )
            self.register_buffer(
                "sampling_eps_max", torch.tensor(config.sampling_eps_max)
            )
    ...
    def forward(
        ...
        if sample_mode:
            assert (
                self.config.attn_backend == "sdpa"
            ), "Sampling only supported with SDPA"
