...
def self_bleu(generations_df, n_sample=1000):
    ...
    for i, row in generations_df.iterrows():
        # gens = row['tokens']
        gens = [
            [str(token) for token in tokens] for tokens in row["tokens"]
        ]  # for gen in row['generations']] {'prompt':"", tokens: [[1,2,3], [3,4,5], [5,6,7], ....]}
        all_sentences += gens
            ...
            list(
                tqdm(
                    pool.imap_unordered(
                        ...
                        random.sample(
                            range(len(all_sentences)), min(n_sample, len(all_sentences))
                        ),
                    ),
                    ...
                    tqdm(
                        pool.imap_unordered(
                            ...
                            random.sample(
                                ...
                                min(n_sample, len(all_sentences)),
                            ),
                        ),
                        total=min(n_sample, len(all_sentences)),
                        smoothing=0.0,
...
def compute_intctr(
    ...
    for text in machine_text:
        ...
        output.logits[:, 1] = -float(
            "inf"
        )  # cardiffnlp/twitter-roberta-base-sentiment's label idx 1 is neutral, which we should ignore
...
def main(generations_file, output_file, metrics, extra):
    ...
    if "ppl" in metrics:
        ...
        for metric in metricset:
            if "ppl" in metric:
                ...
                eval_model = AutoModelForCausalLM.from_pretrained(eval_modelname).to(
                    device
                )
    ...
    if "cola" in metricset:
        ...
        cola_accuracy = fluency_classify(
            generations_df, output_file=output_dir / (output_file + ".cola")
        )
    ...
    if "repetition" in metricset:
        ...
        rep_rate = repetition(
            generations_df,
            eval_tokenizer,
            rep_file=output_dir / (output_file + ".repetitions"),
        )
        with open(output_dir / output_file, "a") as fo:
