...
@click.option(
    ...
    required=True,
    type=str,
    help="huggingface model name to decode from",
)
...
def main(
    ...
    assert (
        batch_size % repeat_sample == 0
    ), "batch_size must be divisible by repeat_sample"
    ...
    with torch.no_grad():
        for i, row in tqdm(
            ...
            if len(text_batch) == batch_size or (i + 1) == len(generations_df.index):
                ...
                text_outputs += [
                    (
                        ...
                        tokenizer.decode(
                            outputs[_i][len(tokens["input_ids"][_i]) :],
            ...
            if (i + 1) % 100 == 0:
                print(
                    f"done {i+1} contexts, took {(time.time()-st)/(i+1)} seconds per context, total generations {len(text_outputs)}",
                    flush=True,
                )
        ...
        for i, row in tqdm(
            ...
            generations_df[f"tokens"][i] = [
                text_outputs[j][2]
                for j in range(i * repeat_sample, (i + 1) * repeat_sample)
            ]  # note this may have mismatched token ids compared to our roberta-based model
            generations_df[f"string"][i] = [
                text_outputs[j][3]
                for j in range(i * repeat_sample, (i + 1) * repeat_sample)
            ]
            # print(row)
