...
def apply_controlling_drift(args, perturbed_inputs_diralpha):
    ...
    if args.ctr_model is None:
        ...
        args.ctr_model = AutoModelForSequenceClassification.from_pretrained(
            ctr_model_name
        ).to(args.accelerator.device)
    ...
    for ctr_i in range(1):
        with torch.enable_grad():
            ...
            perturbed_inputs_simplex_4ctr = torch.nn.functional.softmax(
                perturbed_inputs_diralpha_4ctr, dim=-1
            )
            perturbed_inputs_embeds_4ctr = torch.nn.functional.linear(
                perturbed_inputs_simplex_4ctr,
                args.ctr_model.get_input_embeddings().weight.t(),
            )
            ctr_loss = -torch.nn.functional.log_softmax(
                args.ctr_model(inputs_embeds=perturbed_inputs_embeds_4ctr).logits,
                dim=-1,
            )[:, optimizing_label_index].mean()
            args.ctr_loss = ctr_loss
            ctr_delta = -torch.autograd.grad(ctr_loss, perturbed_inputs_diralpha_4ctr)[
                0
            ]  # indexing 0 because the return is a tuple

        perturbed_inputs_diralpha = (
            perturbed_inputs_diralpha + args.decode_ctr_lr * ctr_delta
        )
...
def logits_uneven_projection(logits, top_p, one_hot_value):
    ...
    filtered_logits = (
        filtered_logits - max_logits + one_hot_value
    )  # max logit gets +5, others keep the same diff with max logit
...
def decode(
    ...
    if args.decode_truncate_len > 0:
        diffusion_input_ids = batch_input_ids[
            :, args.context_size : -args.decode_truncate_len
        ]
    else:
    ...
    for i in range(dec_depth):
        unit_noise = (
            args.noise_manual_scale
            * args.one_hot_value
            * torch.normal(0, 1, size=(batch_size, unit_seq_len, args.vocab_size)).to(
        ...
        t_range.reverse()
        progress_bar = tqdm(
            range(len(t_range)), disable=not args.accelerator.is_local_main_process
        )
        ...
        for t in t_range:
            ...
            (
                ...
                beta_t_til,
                alpha_t,
            ) = get_time_variables(selected_t, total_t, args.accelerator.device)
            beta_t_til = beta_t_til.view(batch_size, 1, 1)
            zt = (
                args.noise_manual_scale
                * args.one_hot_value
                * torch.normal(
                    0, 1, size=(batch_size, unit_seq_len, args.vocab_size)
                ).to(args.accelerator.device)
            )
            ...
            if t % 200 == 0 or t == 1:
                simplex = torch.nn.functional.softmax(
                    xt, dim=-1
                )  # HACK: only for mean of dirichlet, not for sample
                logger.info(
                    f"sigma_t={sigma_t}, training_coef_at_t={torch.sqrt(1 - alpha_t_bar)}"
                )
                logger.info(
                    f"predicted simplex's entropy={torch.distributions.categorical.Categorical(logits=equivalent_score).entropy()}, logit_max,min,mean={torch.max(equivalent_score)},{torch.min(equivalent_score)},{torch.mean(equivalent_score)}"
                )
                ...
                real_token_ids_list = torch.argmax(simplex, dim=-1).view(
                    batch_size, unit_seq_len
                )
                ...
                logger.info(
                    f"t={t} (before +z): {colored(str(sampled_sequences), 'green')}"
                )

                alt_i = 1  # look at the second best candidate
                alt_real_token_ids_list = (
                    torch.topk(simplex, alt_i + 1, dim=-1)
                    .indices[:, :, alt_i]
                    .view(batch_size, unit_seq_len)
                )
                alt_sampled_sequences = tokenizer.batch_decode(
                    alt_real_token_ids_list.clone().detach().to("cpu")
                )
                logger.info(
                    f"t={t} (alt{alt_i+1}): {colored(str(alt_sampled_sequences), 'blue')}"
                )

                logger.info(f"ctr loss: {args.ctr_loss}")
                logger.info(
                    f"non-zero vocab: {torch.count_nonzero(projected_logits > -args.one_hot_value+0.0001) / simplex.size(0) / simplex.size(1)} out of {torch.numel(projected_logits) / simplex.size(0) / simplex.size(1)}"
                )
...
def main():
    ...
    accelerator = Accelerator(
        kwargs_handlers=[
            InitProcessGroupKwargs(timeout=datetime.timedelta(seconds=259200))
        ]
    )
    ...
    if args.train_mode == "decode":
        ...
        logger.info(
            f"Overwriting model_name_or_path ({args.model_name_or_path}) with {args.output_dir}"
        )
    ...
    ):  # resuming job would still break the strict reproducibility, since we are not saving noise states
    ...
    out_json_fn = os.path.join(
        args.output_dir,
        f"ctx{args.decode_context_size}_trunc{args.decode_truncate_len}_depth{args.decode_depth}_ctrlr{args.decode_ctr_lr}_step{args.sigma_num_steps}_topp{args.projection_top_p}_decalg{args.projection_alg}_ctridx{args.ctr_opt_label_idx}_"
        + args.out_fn,
    )
    ...
    if args.train_mode == "decode":
        ...
        if "interactive" in args.remove_noise_mode:
                    ...
                    if accelerator.is_main_process:
                        ...
                        if prompt:
                            ...
                            args.decode_truncate_len = (
                                args.orig_decode_truncate_len - args.context_size
                            )  # compensates for the unknown input context size
            ...
            args.orig_decode_truncate_len = args.decode_truncate_len
            with torch.no_grad():
                for step, _fd in enumerate(
                    fin_data
                ):  # only support batch size 1 since the context size can be different across lines
                    accelerator.wait_for_everyone()
                    ...
                    input_ids = torch.LongTensor(
                        tokenizer.encode(_fd[ctx_field_name], add_special_tokens=False)
                    ).to(args.accelerator.device)
                    args.context_size = len(input_ids)
                    args.decode_truncate_len = (
                        args.orig_decode_truncate_len - args.context_size
                    )  # Han: this compensates for the unknown input context size
                    input_ids = input_ids.unsqueeze(0)
                    ...
                    for _r in range(repeat_sample):
                        ...
                        if _r == 0:  # first sample
                            ...
                            for _i in range(args.per_device_eval_batch_size):
                                ...
                                export_dict["tokens"] = [
                                    history_decode_ids.tolist()[_i]
                                ]
                                export_dict["string"] = [sampled_sequences[_i]]
                                export_dict[
                                    "gold_tokens"
                                ] = diffusion_input_ids.tolist()[_i]
                                export_dict["gold_string"] = gold_sequences[_i]
                                export_list.append(export_dict)
                        else:
                            for _i in range(args.per_device_eval_batch_size):
                                export_list[-(args.per_device_eval_batch_size - _i)][
                                    "tokens"
                                ].append(history_decode_ids.tolist()[_i])
                                export_list[-(args.per_device_eval_batch_size - _i)][
                                    "string"
                                ].append(sampled_sequences[_i])
            ...
            "dump_dataloader" in args.remove_noise_mode
        ):  # some tokens like </s> might be wrongly parsed; for formal evaluation, we directly load the dataloader, not load from jsonl
            # breakpoint()
            dump_json_fn = os.path.join(
                args.output_dir, f"owt_eval_input_ctx{args.decode_context_size}.jsonl"
            )
            export_list = []
            with torch.no_grad():
            ...
            raise ValueError(
                "correct mode should be included in remove_noise_mode, a string separated by pipes"
            )
