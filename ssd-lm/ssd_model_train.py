...
def analyze_perturbed_text_properties(
    args,
    selected_t,
    alpha_t_bar,
    target_token_ids_list,
    perturbed_inputs_simplex,
    vocab2freq,
):
    ...
    subtitution_rate = (
        target_token_ids_list.view(-1).ne(real_token_ids_list.view(-1)).sum().item()
        * 1.0
        / (bs * sl)
    )
    simplex_entropy = (
        torch.distributions.categorical.Categorical(perturbed_inputs_simplex)
        .entropy()
        .mean()
        .item()
    )
    simplex_nll = torch.nn.functional.nll_loss(
        torch.log(perturbed_inputs_simplex.view(-1, vs)), target_token_ids_list.view(-1)
    ).item()
    orig_token_freq = np.mean(
        [vocab2freq[tid.item()] for tid in target_token_ids_list.view(-1)]
    )
    noisy_token_freq = np.mean(
        [vocab2freq[tid.item()] for tid in real_token_ids_list.view(-1)]
    )

    args.noise_analysis_list.append(
        {
            "timestep": selected_t.item(),
    ...
    if args.train_mode == "train" or args.train_mode == "resume":
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                args.context_size = np.random.randint(
                    low=ctx_low, high=ctx_high
                )  # min (1 dec block size), max (max seq length - 1 dec block size)
                ...
                inputs_diralpha = (
                    2
                    * one_hot_value
                    * torch.nn.functional.one_hot(diffusion_input_ids, vocab_size)
                    - one_hot_value
                )

                ######## NOISE ADDITION ########
                selected_t = torch.FloatTensor(
                    np.random.choice(t_list, batch_size, replace=True)
                ...
                (
                    ...
                    beta_t_til,
                    alpha_t,
                ) = get_time_variables(selected_t, total_t, accelerator.device)
                alpha_t_bar = alpha_t_bar.view(batch_size, 1, 1)

                unit_noise = (
                    args.noise_manual_scale
                    * one_hot_value
                    * torch.normal(0, 1, size=inputs_diralpha.shape).to(
                        accelerator.device
                    )
                )
                ...
                if "biased_z" in args.remove_noise_mode:
                    ...
                    perturbed_inputs_diralpha_noexp = (
                        torch.sqrt(alpha_t_bar) * inputs_diralpha
                ...
                if "no_dir" in args.remove_noise_mode:
                    ...
                    if mean_or_protect_for_nan:
                        ...
                        perturbed_inputs_simplex = (
                            dir_model.mean
                        )  # Han: choose between .sample() and .mean?
                else:
                ...
                timestep_embeds = timestep_layer(
                    t_progress.view(batch_size, 1, 1).repeat(1, seq_len, 1)
                )
                ...
                outputs = model(
                    inputs_embeds=diffusion_embeds, output_hidden_states=False
                )
                ...
                if args.loss_mode == "kl":
                    ...
                    loss = (
                        torch.sum(
                            ...
                            * (
                                torch.digamma(a)
                                - torch.digamma(a0).view(batch_size * seq_len, 1)
                            ),
                            dim=-1,
                        )
                    ...
                    loss = torch.mean(
                        torch.linalg.norm(
                            diff_to_be_normed.view(
                                batch_size * seq_len * vocab_size, -1
                            ),
                            ord=2,
                            dim=-1,
                        )
                ...
                ) == 0 or step == len(train_dataloader) - 1:
                    mean_loss = np.mean(train_losses)
                    logger.info(f"train loss: {mean_loss}")
                    train_losses = []
                    if (step + 1) % (
                        args.gradient_accumulation_steps * 1000
                    ) == 0 or step == len(train_dataloader) - 1:
                        accelerator.wait_for_everyone()
                        ...
                        if accelerator.is_main_process:
                            ...
                            torch.save(
                                ...
                                os.path.join(args.output_dir, "embed_sum_layer.pt"),
                            )
                            torch.save(
                                accelerator.unwrap_model(timestep_layer).state_dict(),
                                os.path.join(args.output_dir, "timestep_layer.pt"),
                            )
                            with open(
                                os.path.join(args.output_dir, "completed_steps.txt"),
                                "w",
                            ) as f:
                                f.write(f"{completed_steps}")
                            with open(
                                os.path.join(args.output_dir, "training_losses.txt"),
                                "a",
                            ) as f:
                                f.write(f"{completed_steps} : {mean_loss}\n")
                        logger.info(f"saved model at completed steps {completed_steps}")
                        accelerator.save_state(
                            os.path.join(args.output_dir, "accelerate_ckpt")
                        )
                        ...
                        if (step + 1) % (
                            args.gradient_accumulation_steps * _log_interval
                        ) == 0:
                            ckpt_dir = os.path.join(
                                args.output_dir,
                                f"ckpt{int((step+1)/(args.gradient_accumulation_steps*_log_interval))}/",
                            )
                            if accelerator.is_main_process:
                            ...
                            unwrapped_model = accelerator.unwrap_model(model)
                            unwrapped_model.save_pretrained(
                                ckpt_dir, save_function=accelerator.save
                            )
                            if accelerator.is_main_process:
                            ...
                            logger.info(
                                f"Separately logged the model at completed steps {completed_steps}"
                            )
