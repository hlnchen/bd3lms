...
for seqlen in {1025, 2050}:
    for total_t in {25, 1000}:
        ...
        all_files = glob.glob(
            f"/home/ma2238/bam-dlms/ssd-lm/generated_samples/samples_openwebtext-valid_bs25_l{seqlen}_t{total_t}_SEED*"
        )
        print(
            f'missing seeds {set(range(1,301)) - set([int(f.split("SEED")[1]) for f in all_files])}'
        )
        METRICS.reset()
        ...
        print(
            f"sequence length {seqlen}, total timesteps {total_t}, num samples {len(METRICS.gen_ppls)}"
        )
