...
def decode(
    ...
    for _ in tqdm(range(dec_depth), desc="Blocks", leave=False, disable=not verbose):
        ...
        unit_noise = one_hot_value * torch.normal(
            0, 1, size=(batch_size, unit_seq_len, vocab_size)
        ).to(device)
        xt = unit_noise
        ...
        t_range.reverse()
        progress_bar = tqdm(
            range(len(t_range)), desc="Decoding", leave=False, disable=not verbose
        )
        ...
        for t in t_range:
            ...
            (
                ...
                beta_t_til,
                alpha_t,
            ) = get_time_variables(selected_t, total_t, device)
            zt = one_hot_value * torch.normal(
                0, 1, size=(batch_size, unit_seq_len, vocab_size)
            ).to(device)
            ...
            projected_logits = logits_sampling_projection(
                equivalent_score, top_p=projection_top_p, one_hot_value=one_hot_value
            )
    ...
    return {
        ...
        "context_sequences": context_sequences[0]
...
def main(args):
    ...
    _stdict = torch.load(
        os.path.join(hf_hub_download(args.model_name_or_path, "embed_sum_layer.pt"))
    )
    _stdict = dict(
    ...
    _stdict = torch.load(
        os.path.join(hf_hub_download(args.model_name_or_path, "timestep_layer.pt"))
    )
    _stdict = dict(
        (_k[len("module.") :], _stdict[_k])
        if _k.startswith("module.")
    ...
    model, embedding_sum_layer, timestep_layer = accelerator.prepare(
        model, embedding_sum_layer, timestep_layer
    )
    model.eval()
    ...
    prompt_dataset = (
        datasets.load_dataset(args.prompt_dataset, split="test")
        if args.use_prompt_dataset
        else None
    )
    ...
    for i in tqdm(range(args.num_samples_to_generate), desc="Samples"):
        if prompt_dataset is not None:
            ...
            input_ids = torch.LongTensor([tokenizer.bos_token_id]).to(
                accelerator.device
            )
        ...
        json.dump(
            {
                **vars(args),
                ...
                "generated_samples": [
                    {f"sample_{i}": g} for i, g in enumerate(generated)
                ],
            },
            f,
            indent=2,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--generated_samples_outfile", type=str, default="generated_samples.json"
    )
