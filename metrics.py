...
class Metrics:
    def __init__(self, config=None) -> None:
        ...
        self.eval_ppl_batch_size = self.config.eval.perplexity_batch_size
    ...
    def record_generative_perplexity(
        ...
        for i in range(num_batches):
            ...
            num_strides = math.ceil(
                (samples_batch.shape[-1] - eval_context_size + stride) / stride
            )
            ...
            for i in tqdm(range(num_strides), desc="Sliding Window Gen PPL"):
                ...
                valid_tokens = (
                    sample_chunk[..., 1:] != self.tokenizer.eos_token_id
                ).to(torch.float)
                ...
                if i == 0:
                    ...
                    valid_tokens_accum[..., update_start:end] += valid_tokens[
                        ..., -update_window:
                    ]
            ...
            for i in range(samples_batch.shape[0]):
                _, counts = torch.unique(
                    samples_batch[i], return_counts=True, sorted=False
                )
